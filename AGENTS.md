...
Deps and Antigravity plugins are managed separately via `chai update` (clone/pull deps, install plugins).
...
| Antigravity     | `~/.gemini/GEMINI.md`             | `~/.gemini/config/skills/`         | `~/.gemini/config/agents/`         | `~/.gemini/config/mcp_config.json` | replace key   |
...
- **Sync doesn't touch deps or plugins** — `chai sync` is fast and predictable. `chai update` handles cloning/pulling deps and installing Antigravity plugins.
- **Antigravity plugins** — bundled skills/agents/MCPs installed via `agy plugin install <url>`. Declared under `[antigravity.plugins]`. Only installed when `antigravity` is in `platforms`.
